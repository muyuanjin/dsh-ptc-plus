...
- Treat historical journal recovery as a state boundary, not an availability gate. Reject unproved cells and bindings as recovery evidence, contract to the greatest verified frontier, and conservatively discard anything that may depend on an unknown boundary; when no non-empty frontier is provable, reset to an empty REPL and still execute the current valid cell. Persist and report the contraction once without replaying or claiming to undo historical effects. Damage confined to historical PTC Plus metadata must not permanently block later `run_code`; current-request validation, DSH policy, authority, approval, cancellation, and sandbox decisions remain authoritative errors.
- Keep the default stateful compute environment aligned with what the model can know. A binding may survive compaction or recovery only when both its journal ancestry is reconstructable and its exact provenance remains in DSH's model-visible request surface, or an explicitly selected bounded model-visible state projection makes it knowable. Do not inject such a projection by default merely to retain hidden state. The append-only raw log and UI-only binding metadata are not model knowledge. Use DSH's public ordered surface and formal source relations; do not parse natural-language compaction summaries into state proof. A result-only replacement need not remove a binding when the assistant call containing its source remains visible.
