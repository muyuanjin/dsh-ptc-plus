...
设置卡片、正文 PTC Plus tool view 与 REPL 内存卡片的全部文案注册到 DSH client 的 `settings.ptcPlus` locale 命名空间，随界面语言在中文与 English 之间切换；字段名称与说明的双语文本由 `internal/config-spec.js` 拥有，展示 chrome 文案由 client half 拥有。稳定 REPL 指引保持插件拥有的协议文本，不携带 UI 品牌名。选择当前 `ptc` preset 或旧 Host 的 `code` preset 且插件启用时，Client 会在会话头部以稳定 slot id `ptc-plus-active` 显示原有轻量 `PTC Plus` 标识；悬浮或键盘聚焦时，从公共 `sessionProjections` 的 `ptcPlusRepl` value 显示持久 binding 名称及 variable/function/class/import 类别。运行时只把有界、value-free 的完整 binding inventory 写入并行私有 `meta.dshPtcPlusBindings`；volatile、discarded 或 restore-bearing 结算只写 unavailable，避免冷恢复后把已经不存在的 binding 宣称为当前状态。投影仅以公共 `tool/call` 序号和 `tool/result.sourceEventSeqs` 关联 `run_code` / `edit_run_code` 结算；相关结果缺少或损坏快照时降级为 unavailable，无关工具结果保持现状。不读取或序列化 binding value，不执行代码，不增加模型可见内容。插件启用期间，Client 还通过公共 keyed `tool.call.toolview` 为 `run_code` 与 `edit_run_code` 保留源码/结果视图，并只在已有 presentation metadata 能证明时展示具体功能事件；关闭时注销这两个 key，交还 DSH 原生 fallback。正文不显示 cell、call、恢复边界等计数。该展示不请求历史、不检查宿主 DOM，也不增加或改变 canonical result、tool、prompt、runtime context、journal schema 或迁移；旧日志、缺少或不兼容的 projection service、未知或损坏 metadata 均降级为无 binding inventory，继续会话和核心 runtime 语义不变。当前 `projectionValues.agentPreset` 与旧顶层字段冲突时以前者为准。DSH settings 服务缺省时回退 composition config，原行为不变。
