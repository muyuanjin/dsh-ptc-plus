...
Global User Binding 快照以并行私有 `meta.dshPtcPlusUserBindings` 保存；封闭 journal 只保存该快照的 fingerprint 或显式 `null`，从而验证并行 key 的存在性和内容关系。每次恢复都从条目源码重新验证 identity、声明、binding 列表、durability 和 fingerprint；损坏、缺失或与 journal/源码不一致的快照不能由当前磁盘内容补齐，也不能证明历史 binding，必须按相同的 unknown-boundary contraction 收缩。
