...
8. Global User Bindings 是默认关闭的用户计算输入。关闭时不得读取存储、注入值或新声明、注册管理 RPC/命令或显示管理 UI；下一次宿主允许且接受的步骤可撤销已投递的旧声明；开启时，源码派生的严格快照共同拥有冲突校验、worker 激活和 journal evidence。每个条目可用 `modelContext.includeDeclaration` 选择是否注入已有的源码派生接口（默认开启），用 `instructions` 编写独立提供给模型的提示词（默认空）；不另行编写接口。已启用条目的非空提示词和所选接口从首轮通过追加的 PTC runtime snapshot 投递；`/binding` 保存并启用、会话中途启停、删除或编辑后，下一次宿主允许且接受的请求追加当前配置或撤销旧说明，保持 system、tool schema 与顺序不变。已提交消息和公共 ordered surface 拥有投递、去重与恢复证据，配置未变时不重复追加。该配置只描述 API 与执行时初始化契约，不证明当前值或成功激活，不为组装提示词执行源码。活动声明仍只能来自当前 worker 已成功激活、与请求配置同源且没有 session-local shadow 的完整条目，并通过同一 snapshot 中独立的活动段投递；关闭接口注入同时省略两种声明，但不影响独立提示词或绑定执行。失败 initializer 已发起 program call 时，整个 cell 进入 volatile，不能把缺少该 initializer 源码的成功子集用作 cold replay evidence。模型不获得持久化、候选执行或启停权限。
...
- 每个 cell 创建统一 lease。native tools、`capabilities.*`、`repl.state` 和 `code.*` 在 cell 结束后一起失效；下一次 dispatch 仍由 DSH 重新治理。
- `capabilities.tree/find/inspect` 描述当前 agent scope 的 live tool schemas 以及插件自有 `repl.state` / `code.run` 契约，不调用 capability、不提升权限、不触发辅助模型请求。默认 SDK 展开 explorer，不主动展开两个低频高级 API；Global User Bindings 开启时另包含稳定交接成员。
...
- 已激活的 Global User Binding 以 `namespace` 对象或选定 `top-level` 导出进入普通可写 REPL binding；模块导出保持 live read，同名赋值或重声明形成 session-local 覆盖且不写回全局存储。相对 import 在候选运行与正式激活时都从 `bindings.json` 所在目录解析；导出值被普通 session binding 保留后，其闭包所需的解析基准和动态 program namespace bridge 保留到 worker 结束。请求自带的 program namespace 与 error class 始终优先；同名全局条目只在该请求中激活失败并产生诊断，不能覆盖宿主值或阻止独立 cell。binding module 所需的 program namespace 若与既有 worker global 同名，本次 cell 显式失败，不能替换 Node intrinsic。不可写、保留或来源不明的名称继续由既有重声明策略拒绝。
- cell 包裹前的 AST 重写适配模块语法（cell 是 async function body，模块声明在函数体内非法），全部默认开启（`autoRewriteImports` / `autoStripExports` / `autoSplitRedeclarations`，可关闭）：原始 Program 在 lowering 与 preload 前完成 scope validation，同一 cell 的 value import 重名按模块 binding 规则拒绝；worker 按源码顺序从 session cwd 预加载并静态链接 `import`，session 未记录 cwd 时回退到 process cwd；cell 通过持久 module namespace slot 捕获结果，named/default alias 保持 live read 与只读 binding 语义；value import alias 活跃时，无法由静态 AST 保真的 direct `eval` 与 `with` 在任何 module preload 前拒绝；顶层 `export` 修饰符剥离（声明保留、default 转 `__default`、re-export 转预加载的副作用模块、type-only 擦除）。跨 cell 重声明属于 REPL 便利策略，由 `internal/repl-convenience.js` 隔离实现：`looseTopLevelRedeclarations` 独立控制顶层变量替换及混合新旧名称的解构 lowering；默认开启的 `looseTopLevelFunctionClassRedeclarations` 只把重复的 named function/class 声明在原声明位置改写为对已有可写 binding 的赋值，显式关闭时恢复原有拒绝行为。不可写 import、未被变量便利策略降为 `let` 的 `const`、保留名称和来源不明的 binding 在执行前拒绝；同一 cell 和 block scope 规则仍由原始 JavaScript/TypeScript Program 验证。改写来源平行记录在 `meta.dshPtcPlusRewrites`（journal 封闭 schema）；成功改写不产生 runtime context，失败或缺少有效 journal 时才用 `tools:ptc-plus-rewrite-info` 告知不确定的后续动作。
...
- PTC 模式的 code-only direct-tool projection 固定暴露 `[run_code, edit_run_code]`，但 DSH 默认只投影 `run_code` 不是插件必须继承的架构约束。DSH owner 为一个 agent composition 选择一次 `native`、`ptc` 或 `both`；PTC Plus 必须在自己的 agent-scoped presentation effect 生效前捕获首次可证明的 composition mode，并在该 agent 生命周期内只从这项事实派生 direct projection、native-call normalization 与 dispatch rejection。非空 `tools:ptc-only` rule 证明 `ptc`，同名空 section 证明 `both`；`tools:code-only` 仅在 `tools:ptc-only` 缺失时作为同语义兼容别名，两者同时存在时以 `tools:ptc-only` 为准。插件为执行 `edit_run_code` 调用的 `tools.presentAs('both')` 只是宿主执行效果，不能成为后续模式证据。PTC Plus 通过 DSH 的公共工具注册与 agent 级呈现接口真实注册并呈现 `edit_run_code`；无关 native tools 仍只作为程序内 `tools.*` 能力提供。`edit_run_code` 保留模型原始调用与参数，使用独立 editor 对同一未结束 turn 中调用发起时最近的可编辑 cell 原子应用至多 16 个唯一、非重叠的字面替换，或至多 16 条带显式预期匹配数的正则替换；发起后的其他 settlement 不追溯改变该目标。普通 edit 可省略目标前置条件；携带 `expected_target_call_seq` 时，捕获目标的 call sequence 必须相同，否则不编辑、不执行。目标既可以是被拒绝或失败的 cell，也可以是已成功执行但仍需微调的 cell；成功 edit 的派生源码成为下一目标。完整物化源码只进入插件派生的 `run_code` 执行和可恢复日志元数据，不替换或重复进入模型 assistant history；执行结果由外层 `edit_run_code` result 表达。调用身份、派生执行、journal、恢复与 UI 展示必须保持可区分；`llm/stream` 不得重命名合法的 `edit_run_code`，也不得添加 edit 专用 runtime context。所有匹配、长度、capture、展开和 CPU 预算仍由 editor 统一拥有；失败、取消、未知 effect 与冷恢复语义必须从 session log 重建。

## Journal facts

`version: 5` journal 是封闭 schema，只包含：由 `variableRedeclarations` 与 `functionClassRedeclarations` 组成的 binding policy、rewrite policy、固定源码 lowering 代际的 module semantics、Global User Binding snapshot fingerprint 或显式 `null`、status、call transcript、state operations、confirmed no-op call sequences、diagnostics、completion 与 optional volatile reason。两组 policy 固化该 cell 的 binding 与 AST rewrite 开关，`moduleSemantics.defaultExportBinding` 固化 `export default` 采用 `legacy-variable` 或 `live-readonly` 绑定模型；冷恢复不读取当前 profile 的开关，也不从源码猜测 lowering 代际。v1-v4 输入迁移时保留各自既有语义，并把尚无 Global User Bindings 持久关系的历史格式固定为 `null`。call transcript 包含 namespace/member、PTC Value Graph 编码的 args/value 或 error，以及连续 settlement 序号；不包含推测的 effect 或 completeness。基础设施终止时 calls 清空；若仍有未结算 binding，`discarded.volatileReason` 保存最先观察到的 `global.member` possible-effect boundary。

Global User Binding 快照以并行私有 `meta.dshPtcPlusUserBindings` 保存；封闭 journal 只保存该快照的 fingerprint 或显式 `null`，从而验证并行 key 的存在性和内容关系。快照 v2 记录固定编译器的 `transform` 标识并纳入 fingerprint，恢复先验证转换代际，再从条目源码重新验证 identity、声明、binding 列表、durability 和 fingerprint。v1 未记录原生转换器代际，其非空快照不能证明历史模块值，必须收缩到此前可验证的 frontier；空 v1 快照保持原 fingerprint，可继续证明不涉及全局模块的历史。损坏、缺失、转换代际不受支持或与 journal/源码不一致的快照不能由当前磁盘内容补齐，也不能证明历史 binding，必须按相同的 unknown-boundary contraction 收缩。该过程不修改全局绑定存储，当前合法 cell 仍按当前配置执行。

两阶段确认通过 `run_code.output.presentationMeta` 与 `tools/result` 完成。已执行但最终 journal 缺失或变化时，live state 单调降为 volatile；未进入 runtime 的 call 由后续 `confirms` 证明为 no-op。损坏或缺失历史形成 unknown boundary，cold recovery 不把边界内外无法证明关联的 cell 或 binding 当作已恢复状态。恢复从当前请求所需的 head 向 parent 收缩到最大可证明 frontier；缺少更细粒度依赖证据时按 cell 与依赖后缀丢弃，最差重置为空 REPL，然后执行当前合法 cell 建立新分支并投影一次可行动提示。

durable node 重放失败时，插件把本次收缩记录为当前已结算 `tool/result` 的私有 `meta.dshPtcPlusRecoveryBoundaries`，记录失败 call 与其已验证 parent frontier；call/result 保持不变。boundary 必须在 event sequence 参与排序前完成规范化；损坏 boundary 不得静默消失、参与排序或证明任何旧状态，而应收缩可恢复状态，不能仅因历史插件 metadata 损坏而拒绝当前合法 cell。日志折叠删除失败或不可证明的 node、binding 与依赖后代，重算仍可重建的 checkpoints，重置 worker 后逐级验证更早 frontier，必要时到空 REPL，随后执行当前 live cell。新 durable node 必须连接到 worker 实际重建的 frontier；历史 program binding effect 不重新派发。日志中的 `ptc-plus/recovery-boundary` 事件只能通过显式、非破坏迁移转换后再交给 DSH persistence reader。

recovery frontier 还是结构可恢复状态与模型可知状态的交集。DSH append-only log 保留审计和 journal evidence，DSH ordered surface/实际 request history 拥有模型可见性；插件不能因为 raw event 仍存在或 Client 卡片仍能展示 binding，就把已被 surface compaction 遮蔽且没有模型可见结构化投影的状态继续暴露给代码。surface replacement 只裁剪 tool result 而保留携带源码的 assistant call 时，模型仍有精确 provenance，不能机械删除该 cell。surface generation 收缩必须在下一 cell 前同样作用于现存 live worker，而不只作用于 cold replay。
...
DSH 原生 tool guidance 与 typed SDK 保持 owner 提供的内容。PTC Plus 修改 `run_code` 的连续 REPL 说明，追加 `capabilities` explorer 声明，并在 Global User Bindings 配置开启时声明稳定的 `code.submitBindingDraft`，并在使用 code-only direct-tool projection 的 session-bound PTC request 中增加真实注册的 `edit_run_code` schema。稳定指引只保留 cell async body、模块适配、binding continuity、能力发现和 execution-world 边界；不假设 Windows、WSL、POSIX、shell、package runner 或某个 native tool 存在。改写反馈和按需恢复 tip 只有在事实不已由 call/result 表达且确实改变下一步决策时才使用带 `ptc-plus` 来源的独立消息；当前状态用有界 `snapshot`，tip 用带 trigger/ordinal 的 `notice`。快照只替代 PTC 旧状态，不替代 task、Skill、tool result 或其他 producer；空快照明确撤销旧声明。投递经公共 assembly witness 与 accepted `agent/pre-step`，遵守 runtime-context suppression，从已提交历史和公共 ordered surface 重建，不把待投递消息算作已送达；edit 身份、参数、结果和目标消费由真实 call/result 表达，runtime context 不承载 edit 专用信息。恢复 tip 受 `tipCooldownMessages` 与 `tipEscalationFailures` 约束；成功 cell 重置未解决计数。新 agent 或 Cordis 重新启用代际若从规范 journal 观察到历史 Cordis transcript，则用固定命名 context 将 recorded value 限定为历史数据，并要求在依赖旧 ID、状态或 capability observation 前执行新的成功 Cordis Inspect；该 context 不重放 effect，也不声称进程内状态必然丢失。
...
普通成功 cell 不产生 PTC warning/note。宽松重声明和首次进入 volatile 都是正常运行状态；后者只进入 journal 与 `repl.state(list)`。只有执行失败或 cold recovery 实际跳过历史状态时才投影可行动诊断。同一失败指纹在单个 kernel 内连续出现 3 次时，失败 cell 追加一次分类提示：可证明的 binding 错误使用 `PTC-W001`，其中 lexical 错误指向可见源码、作用域与初始化，只有 worker-owned capability 错误建议 live capability 探查；其他错误使用 `PTC-W002` 且只要求重新检查其具体 cause。附加提示沿用失败 cell 的 stateEffect，不声称此前语句未生效。成功或不同失败会重置计数；提示不重写原始错误。
