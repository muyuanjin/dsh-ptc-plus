...
In an enabled `ptc` session, the header shows a compact green `PTC Plus` activity indicator. Hover, keyboard-focus, or click it to see the names of current persistent bindings, with distinct colors for variables, functions, classes, and imports. The card occupies the browser top layer so a conversation sidebar cannot cover it, and long lists scroll inside the card. It never reads values or executes code. Volatile, recovery-bearing, or incompletely settled results show the inventory as unavailable instead of claiming bindings that cold recovery cannot reproduce; unrelated native-tool results do not clear an already validated list. `run_code` and `edit_run_code` remain titled Code and Code edit in the conversation body: source and result remain expandable, with TypeScript syntax highlighting and copy support when the DSH client exposes its `CodeBlock` primitive. A feature name and useful detail appear only when existing result metadata proves that the feature affected this execution. Examples include the imported module adapted for the REPL, stripped exports, a split mixed redeclaration, safe edit execution, a successful `code.run`, a completed durable recovery, or a `repl.state` operation. The UI does not show process-retention notices or counters for REPL cells, program calls, recoverable cells, or recovery boundaries. Turning the plugin off unregisters both keyed rows and restores DSH's native fallback.
...
These views are presentation-only. The body registers DSH's public keyed `tool.call.toolview`; the memory card reads a public session projection driven by a value-free binding snapshot stored with each result. They add no tool, prompt text, runtime context, session event type, journal field, history request, or migration step, and never serialize binding values into the log. Malformed or unknown body-feature metadata is ignored; a memory snapshot missing from or malformed on a correlated REPL result fails closed to unavailable. An older log without a memory snapshot therefore shows the unavailable state while retaining source, result, lifecycle state, and resumability. The body does not infer ordinary loose redeclarations, canonicalized native calls, official Cordis ownership, or selected recovery tips from source text, member-name prefixes, or settings because the Client has no result-local provenance for them. Installing, removing, or hovering these views therefore does not change model requests or make an otherwise readable or resumable session incompatible.
