...
The Global tab in a PTC session can inspect entry source and prefill `/binding new <requirement>` or `/binding edit <id> <requirement>` for the current Agent. The Agent receives one request-scoped draft handoff and cannot save, run, enable, remove, or import entries. The Host constrains draft UI operations with an opaque locator published only through the current session's private projection, and removes the draft when the session or agent ends or the feature is disabled. A saved draft remains disabled; the user runs and enables it separately in Settings. The candidate worker isolates session state, but its code still has DSH process authority and may produce irreversible Node/OS effects.
...
After cold recovery or re-enabling Cordis, recorded Cordis values remain journal data but do not prove that process-local Plugins, Runs, approvals, or earlier Inspect observations are still live. PTC Plus adds a bounded recovery context until a new successful Cordis Inspect call validates the current process.
