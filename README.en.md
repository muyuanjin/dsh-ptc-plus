...
After a catalog revision conflict, Reload remains available during editing and retrieves the latest catalog and revision while preserving unsaved source and field edits. Review the draft and click Save again to retry; reloading never overwrites or automatically submits the draft.
...
The settled result waits at most 250 ms for a preview. Large-object inspection may extend the next cell's queue wait. Only unfinished observation is exempt from the next cell's compute and wall budgets; other blocking work, including background user callbacks, retains timeout protection, and waiting remains cancellable. Preview time cannot cause the next cell to time out and lose its bindings.
