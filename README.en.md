...
After cold recovery or re-enabling Cordis, recorded Cordis values remain journal data but do not prove that process-local Plugins, Runs, approvals, or earlier Inspect observations are still live. PTC Plus adds a bounded recovery declaration until a new successful Cordis Inspect call validates the current process. Recovery declarations, activated global-binding declarations, and on-demand tips use independently sourced PTC Plus messages that honor Host runtime-context suppression. Their changes do not resend other plugins' unchanged context, and obsolete state declarations are explicitly withdrawn.
