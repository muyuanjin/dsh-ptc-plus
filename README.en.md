...
Choose **PTC Plus settings** in the sparkle menu beside the composer to open the complete settings dialog directly. The host's native entry remains available. Depending on the DSH interface, open **Settings → General → PTC Plus**, find the plugin on the side-bar **Plugins** page and choose **Configure**, or use the older **Settings → Plugin configuration → PTC Plus** entry. If an upgrade leaves settings unavailable and the shortcuts missing, reinstall the current plugin and restart DSH; see [Troubleshooting](docs/installation.md#troubleshooting). The main switch controls the plugin; other settings are grouped by purpose:
