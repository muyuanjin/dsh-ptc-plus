...
可以从输入框旁的星光菜单选择 **PTC Plus 设置**，直接打开完整设置窗口。也可以打开侧栏 **插件 → 已安装 → dsh-ptc-plus**，在本插件行点击 **配置**；旧版宿主的入口为 **设置 → 插件配置 → PTC Plus**。升级后若提示设置服务不可用且快捷入口消失，重新安装当前插件并重启 DSH，详见[故障排查](docs/installation.md#troubleshooting)。星光菜单上方可启停全局绑定，下方将 Agent 编写操作与绑定管理、插件设置分组排列。总开关控制插件，其余设置按用途分组：
