...
保存遇到目录版本冲突时，可在编辑中点击“重新加载”取得最新目录与 revision，未保存的源码和字段编辑会保留。检查后再次点击保存即可重试，刷新不会自动覆盖或提交草稿。
...
当前会话使用 `ptc` 或兼容的 `code` preset 且插件启用时，会话增加与“对话”“轨迹”同级的 **REPL** 页签；切换到其他会话或关闭插件时自动移除。“会话绑定”展示名称、定义来源和结算后的有界值预览，并标注观察时间、截断与不可读取状态。超出展示限额的条目会被省略，其他条目的预览照常显示。TypedArray、Buffer 和 String 包装对象的值标为不可读取，避免预览枚举全部索引；普通字符串仍提供有界文本预览。预览不触发 getter、Proxy trap 或用户格式化代码，也不执行会话代码；它不是实时完整快照，不进入模型上下文或 journal，不影响恢复与 binding 保留规则。
...
预览最多等待 250 ms，未及时完成时先返回已结算的结果。大型对象检查可能延长下一 cell 的排队等待，只有仍在执行的观察不计入下一 cell 的计算与墙钟预算；后台用户回调等其他阻塞仍受超时保护，等待期间可取消。预览耗时不会导致下一 cell 误超时并清空绑定。
