...
本插件通过官方 DSH 已发布的公共扩展接口安装和运行，不修改 DSH 源码或安装包，也不要求定制宿主。具体边界见[安装指南](docs/installation.md)。
...
“显示 REPL 页签”和“显示绑定编写按钮”默认开启，分别控制会话页签与输入框快捷入口。隐藏页签后仍可从设置管理全局绑定；隐藏编写按钮后仍可使用 `/binding` 命令，已有草稿的访问入口仍保留。整个全局绑定能力仍由“全局用户绑定”开关控制。
...
开启后，只要当前 session 的实际命令目录提供 `/binding`，输入框工具栏就会在首轮前显示星光图标；点击可预填 `/binding new `，已有输入不会被覆盖。也可以直接输入 `/binding new <需求>` 或 `/binding edit <id> <需求>`，DSH 会提供命令匹配和参数提示。命令启动 Agent 编写后立即完成并清空输入框；对话历史中的绑定编写请求保留需求和状态，接受后的完整草稿改在当前输入框上方展开。Agent 收到完整字段说明、调用范例和依赖解析基址，通过 `run_code` 内稳定的 `code.submitBindingDraft({requestId, entry})` 交接一个停用内存草稿；每次请求仅接受一次有效提交，普通 REPL 声明不会成为全局草稿。进入和结束编写不改变同配置下的 system 与工具声明。

草稿就绪后，输入框上方的面板默认展开并提供“保存为停用”“保存并启用”和“丢弃草稿”，均由用户决定。面板悬浮在输入区上方，展开不顶高正文，长内容在内部滚动；点击整条标题栏或用 Enter/Space 可折叠、展开，箭头同步变化。有草稿时，输入框的星光按钮显示数量角标；悬停或点击打开草稿菜单，选择名称即可重新打开面板，也支持键盘操作。菜单合并了“编写新绑定”入口，不再增加独立草稿按钮。每个会话目前只保留一份当前草稿；隐藏编写快捷入口后仍可访问已有草稿。关闭面板不保存、不丢弃、不取消编写。保存或丢弃确认成功后面板自动关闭；失败或结果待确认时保留当前展示。原历史请求中仍可展开检视当时接受的精确源码，回执可随日志重建，后续同 ID 的修改不会替换这份历史。保存并启用原子写入启用条目，但当前会话是否成功激活仍由 runtime 证明。模型收到保存事实，下一次允许的请求获得当前配置说明，初始化失败由执行结果报告；`repl.state` 管理命名 checkpoint，不能当作绑定目录。目录版本冲突后，卡片重新读取权威状态，保留仍有效的草稿供用户检视并重试。
...
当前会话使用 `ptc` 或兼容的 `code` preset 且插件启用时，会话增加与“对话”“轨迹”同级的 **REPL** 页签；切换到其他会话或关闭插件时自动移除。“会话绑定”展示名称、定义来源和有界值预览，并标注观察时间、截断状态；缺少预览显示“尚未观察”，无法安全读取的值显示“不可读取”。超出展示限额的条目会被省略，其他条目的预览照常显示。数字、布尔值、字符串、null、undefined 和至多 128 位的 BigInt 可直接预览；更大的 BigInt 只显示大小超限提示。当前 Node REPL 通过存储探测时，顶层 `await` 产生的变量同样可读。数组读取前五个自有槽位，因未检查额外属性而始终标记预览不完整；普通对象、TypedArray、Buffer、String 包装对象等其他值标为不可读取，不枚举全部属性。预览不触发 getter、Proxy trap 或用户格式化代码，也不重跑会话代码；它不是实时完整快照，不进入模型上下文或 journal，不影响恢复与 binding 保留规则。

REPL 在同一页展示会话绑定和完整全局绑定工作台，无需切换内部页签。会话清单支持名称搜索和类型筛选，选中后在检查区查看带高亮与复制按钮的定义及对应值预览；空清单使用紧凑的全宽空状态。观察区可见时，后续执行会采集可选预览；打开页面或清单更新后若没有预览，则向现存 worker 按需读取一次。读取最多等待 250 ms，不启动或恢复 worker、不重放历史，也不后台轮询。工作区隐藏普通消息输入框，也不响应对话宽度拖拽；切回对话后恢复输入框和未发送草稿，审批、提问等宿主交互仍正常显示。

![REPL 工作区：会话绑定检查与全局绑定管理](assets/ptc-plus-repl-workspace-zh.png)

*REPL 工作区：上方检查会话绑定，下方管理全局绑定并使用 TypeScript 代码控制台。*

预览最多等待 250 ms，未及时完成时先返回已结算的结果。只有 worker 已确认开始且尚未完成的观察，可暂缓下一 cell 的计算与墙钟预算；已经开始的预算不会暂停或重置。后台用户回调等其他阻塞仍受超时保护，等待期间可取消。预览的展示等待超时本身不会清空绑定。
