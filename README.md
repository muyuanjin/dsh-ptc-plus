...
可以从输入框旁的星光菜单选择 **PTC Plus 设置**，直接打开完整设置窗口。宿主原生入口也保留；根据 DSH 界面，可打开 **设置 → 通用设置 → PTC Plus**，在侧栏 **Plugins** 页面找到本插件并点 **Configure**，或使用旧版的 **设置 → 插件配置 → PTC Plus**。升级后若提示设置服务不可用且快捷入口消失，重新安装当前插件并重启 DSH，详见[故障排查](docs/installation.md#troubleshooting)。总开关控制插件，其余设置按用途分组：
