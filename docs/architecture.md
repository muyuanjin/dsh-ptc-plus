...
`autoDescribeRunCode` 默认开启；schema 投影允许省略外层 `run_code.description`，执行桥向 DSH 参数校验器提供派生的固定摘要，并仅通过 presentation metadata 持久保存该展示摘要。原始调用参数、cell 与嵌套 native 参数保持不变；关闭后恢复 DSH 的 required 字段与严格校验。
