...
Host half 通过 DSH 公共 settings 服务注册 `ptc-plus` 命名空间，优先使用 provider-owned `installSection()`，并在旧 Host 上适配根模块的 `installSettingsSection()`；两条路径共享同一 fallback、watch、rollback 与 disposal 语义。字段清单、默认值与校验来自 `internal/config-spec.js`。
...
不注册 runtime、hook、tool surface 或 system prompt section；设置卡片显示“已启用/已停用”，当前 `ptc` 与旧 Host `code` preset 会话在头部单独显示 `PTC Plus` 指示器。稳定指引不包含 UI 品牌名。
`autoDescribeRunCode` 默认开启且只控制请求绑定的本地执行策略。无论开关状态如何，投影都保留 DSH 原生 required `run_code.description`，tool schema、tool order 和 system sections 保持字节稳定。开启时，执行桥只向本地 DSH 参数校验器提供派生的固定摘要，并仅通过 presentation metadata 持久保存该展示摘要。原始调用参数、cell 与嵌套 native 参数保持不变；关闭时继续由 DSH 严格校验外层摘要。
