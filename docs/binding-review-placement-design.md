...
公开 `conversation.input.left` 只注册一个星光图标，统一编写与当前草稿访问。没有草稿时，点击仍预填 `/binding new `；有未处理候选时显示数量角标，悬停、点击或方向键打开官方 `Menu`，按名称与状态选择候选后展开面板。菜单中的“编写新绑定”沿用既有命令可用性与输入保护。当前 Host 每会话只保留一份当前草稿，因此角标为 `1`，不把历史候选计为待处理草稿。草稿访问不依赖命令服务；隐藏编写快捷入口时保留已有草稿的星光图标，但不显示新建操作。插件与全局绑定功能开关仍统一控制整个入口。

菜单使用公共 Portal 能力避开输入区裁切，由原生菜单处理定位、移出宽限、外部点击和 Escape；点击与方向键打开时聚焦首项，Tab 可到达新建操作。候选变化、会话切换、功能关闭、卸载或普通输入区被接管时关闭菜单；普通刷新不关闭菜单。菜单选择必须向同一 review controller 传递精确候选，不能把旧项点击重定向到新候选。原请求条目中的入口也只能打开其精确对应的当前候选；旧请求只允许查看自己的只读历史。展示能力暂不可用时，不提供实际无法打开面板的操作。
