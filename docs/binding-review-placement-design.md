...
用户关闭时，焦点移到就近的星光按钮，聚焦本身不弹出菜单；关闭已处理回执且没有该入口时回到输入框。异步成功移除当前聚焦的打开入口时也应保持合理焦点，不能将其留在已移除节点；宿主接管时不从审批界面抢焦点。折叠按钮提供 `aria-expanded`，被隐藏的内容不得继续接受键盘焦点。
