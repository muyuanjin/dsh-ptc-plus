...
菜单使用公共 Portal 和 `getAnchorRect`，以包含原生可编辑区域的最近输入容器上沿定位，按实际可用空间限制列表高度；全局绑定与新建、管理操作共享原生滚动区域，较矮视口下仍可逐项访问。菜单由外部点击或 Escape 关闭，移出触发器时保持打开，便于越过输入区选择条目；点击与方向键打开时聚焦首个可用操作，Tab 可到达新建和管理操作。候选变化、会话切换、功能关闭、卸载或普通输入区被接管时关闭菜单；普通刷新不关闭菜单。菜单选择必须向同一 review controller 传递精确候选，不能把旧项点击重定向到新候选。原请求条目中的入口也只能打开其精确对应的当前候选；旧请求只允许查看自己的只读历史。展示能力暂不可用时，不提供实际无法打开面板的操作。

全局目录由现有 `list` Remote 按菜单打开动作读取，不轮询；显式“重新加载”调用 `reload` 重新读取存储文件并清除已修复的读取错误。启停沿用 `enable`/`disable` 与 `expectedRevision`。写入期间禁用重复切换，关闭菜单不取消已发出的操作。冲突或失败后撤下过期目录，用户重新读取后再决定是否重试，不自动重发写入。重连和组件释放使旧响应失效；菜单读取、显示和启停配置不执行绑定源码。菜单明确区分全局启用配置与待处理草稿，角标只统计草稿。
