...
`userBindingsEnabled` 关闭时不渲染任何 Global User Binding 管理界面，Host 也不注册对应 RPC 或 Agent 命令。开启后，设置卡片追加完整工作台：条目列表、源码编辑、scope 和用途、派生声明预览、验证、revision-checked 保存、启停、删除、从本地 `.ts` 文件导入、以及候选导出调用。候选运行区始终显示 Node/OS effect 警告；独立 worker 不等于安全 sandbox。
...
PTC session 的头部弹窗在功能启用时增加 Session 与 Global 页签。Session 页签保留当前可复用 binding 的只读检查；Global 页签列出全局条目，可展开查看精确源码，也可把 `/binding new ` 或 `/binding edit <id> ` 预填到空 composer。composer 已有非空草稿时保留原文并显示反馈；公开接口不提供 focus 时，Client 不访问宿主 DOM 强制聚焦。窄弹窗不复制完整编辑器、候选运行、启停、导入或删除功能，这些操作由设置工作台拥有。

Agent 提交的有效候选只显示为接受该提交的 session 的内存草稿。Connection RPC 的公共 handler 不提供 caller/session identity，因此 Host 不信任 Client 传入的 session ID：接受提交时生成不可猜测的 locator，只把 locator 放进外层 `run_code` 的私有 presentation metadata，再由该 session 的 `ptcPlusBindingDraft` projection 交给 Client；源码仍只存在 Host 内存。弹窗凭 locator 读取、丢弃草稿，或以当前 catalog revision 原子保存为停用条目；保存开始后同一草稿的并发丢弃会返回 busy conflict，而不会显示与持久状态矛盾的成功。保存并不运行或启用代码，Agent 也无法调用这些 UI 操作。会话结束、功能关闭或 agent disposal 会删除未保存草稿并使 locator 失效；轮次结束只撤销一次性交接能力，已经被接受的内存草稿继续等待用户决定。

`enhancedToolView` 默认开启并即时生效。开启时 PTC Plus 通过公共 keyed tool-view surface 为 `run_code` 与 `edit_run_code` 提供增强行，并在可用时使用 DSH 公共 `DisclosureRow`/`CodeBlock` primitive，缺少某项 capability 时使用插件自有的等价降级；关闭时立即注销这两个 keyed view，由 DSH 原生 generic row 负责布局、状态、代码高亮和输入/输出卡片。该开关只影响 Client 展示，不改变工具、prompt、runtime 或 session 语义。
...
设置卡片、正文 tool view 与 REPL 可复用 binding 卡片的全部文案都注册到 DSH client locale 的 `settings.ptcPlus` 命名空间，随当前界面语言在中文与 English 之间切换；字段名称与说明的两种语言文本同样来自 `internal/config-spec.js`（`label`/`labelEn`、`description`/`descriptionEn`，某一字段的说明要么两种语言都有，要么都没有），展示 chrome 文案由 client half 拥有。稳定 REPL 指引不承载 UI 品牌名。启用且会话选择 `ptc` 或兼容的 `code` preset 时，`conversation.session.header.actions` 以稳定 id `ptc-plus-active` 显示简洁的 `PTC Plus` 标识；当前 session projection 与兼容顶层字段冲突时以前者为准。关闭时不注入任何 PTC 指引或工具 surface。
