...
Every program namespace is leased to one cell. Captured namespace objects or member functions expire when that cell ends, including calls made later by retained Global User Binding continuations.
...
In an enabled PTC session, `/binding new <requirement>` and `/binding edit <id> <requirement>` start a normal Agent turn with a request-specific authoring Skill and one `submitBindingDraft` handoff tool. An edit receives the exact stored entry and must preserve its stable ID. The first valid submission becomes one memory draft with `enabled: false`; duplicate, late, conflicting, and post-lifecycle submissions fail. The handoff is removed after acceptance, turn stop, agent error, replacement, disablement, or disposal. The Agent cannot persist, execute, enable, remove, import, or inspect unrelated bindings. Because Connection RPC exposes no caller/session identity, the Host mints an opaque draft capability and publishes only that locator through private result metadata and the accepting session's `ptcPlusBindingDraft` projection; draft RPC never trusts a payload session ID. Agent/session disposal, feature disablement, and owner disposal invalidate the draft. Saving is a separate authenticated UI action that atomically claims the draft before revision-checked persistence; concurrent discard reports a busy conflict, and new drafts use create-only storage so they cannot replace a concurrently created entry.
