...
The current enabled snapshot is the desired input for the next cell. The `tools:ptc-plus-user-bindings` section in the independent PTC state snapshot contains only complete entries that the current session worker has already activated successfully, that remain source-identical in the desired snapshot, and whose call identifiers still have `user-global` provenance. New and updated entries cross one activation cell before becoming model-visible; failed evaluation, request-owned namespace/error-class conflicts, and session-local shadows cannot advertise unavailable declarations. The stable `tools:sdk` prefix and direct-tool schemas do not change with binding contents. A request-owned program namespace or error class takes precedence over a same-name user entry; that entry produces an activation diagnostic and is omitted from the settled snapshot without blocking independent code. If a failed initializer issued a program call, the cell becomes volatile because the successful-entry snapshot is not complete replay source for that call transcript.

Namespace members and top-level exports preserve ECMAScript module live reads until assignment or a permitted redeclaration creates a session-local shadow. A shadow is not written to the global document and suppresses the complete source entry from later model projection. Updates, disablement, and removal affect unshadowed names from the next submitted cell. Entry imports resolve from the directory containing `bindings.json` during both candidate evaluation and session activation, independently of the session cwd. Binding modules can use program namespaces present in the current request through a stable bridge; new namespace names are installed as they appear, absent namespaces expose no members, and each invocation uses its originating cell lease. A namespace that has the same name as any existing worker global fails explicitly before bridge installation, so Node intrinsics cannot be replaced and a failed installation cannot leave a partial bridge. Once an exported value may have escaped into an ordinary session binding, its synthetic-module resolution base and installed bridges remain available for that worker lifetime; a fresh disabled, empty, or failed-only worker remains unchanged.

The Settings workbench owns full management: source editing, validation, declaration preview, revision-checked persistence, import of a local `.ts` file as a disabled entry, activation, removal, and candidate execution. Candidate execution evaluates source in a separate bounded worker and optionally invokes one exported function with a JSON argument array. It does not modify a live session or enter the session journal. Worker separation protects session state, not the host: source and imports run with DSH process authority and may create irreversible Node/OS effects before cancellation, timeout, or failure.
...
Stored, enabled, and successfully activated are distinct facts. Global UI state reports the catalog's enabled flag. Active model declarations require the current worker's proven activation with no shadow; a save notice alone proves no activation. `repl.state` is a checkpoint function and its `list` names are checkpoint names, not bindings. Availability questions use active declarations or a side-effect-free observation of a known name, without write/delete smoke tests. Effectful verification must be authorized by the task and use exclusively created resources whose ownership is established before cleanup.
