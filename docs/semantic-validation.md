...
候选环境测试曾出现一次文件级异常退出，未留下底层退出诊断，独立复测尚不能复现。该问题由进程稳定性验证跟踪，原因仍未确认，不作为已经修复的语义缺陷处理。下一步应给完整测试运行保留原始进程诊断，并在上述有限压力矩阵中复现、归因；一旦复现，按实际 owner 修复。所有测试和资源限额继续执行，不允许重试自动掩盖失败。
