...
macOS and Windows runners abort whole coverage-instrumented test files natively; the file ends without a failing test and `coverage/run-*.tap` records its process exit code or signal instead. A local Windows reproduction reached `exitCode 3221225477` (an access violation) when coverage and the compiler bytecode snapshot were both enabled; coverage alone and the snapshot alone did not reproduce it, Ubuntu stayed green, and the macOS runner already runs one file at a time. This accepted limitation belongs to the coverage gate: `.github/workflows/ci.yml` sets `DSH_PTC_TEST_CONCURRENCY: 1` for every platform and retries a failed run only when its own TAP record shows a test file that ended abnormally (a signal or an exit code other than 1), printing that excerpt; any other failure, including a file that cannot load, fails the job immediately. Revisit when Node's coverage collection, the compiler bytecode snapshot, or that signature changes.
