/** Historical legacy-v1 lowering, retained from c51616c8d6c8:cell-analysis.js. */
import { ModuleRewriteError, rewriteModuleImportsExports } from './legacy-cell-rewriter.js'
import { parseExecutableCell } from './legacy-cell-parser.js'
import {
...
import { rewriteReplRedeclarations } from './legacy-repl-convenience.js'
...
export class LegacyPreflightError extends Error {
...
function staticModuleClassification(moduleLoads) {
  ...
  for (const load of moduleLoads) {
    ...
    if (classification.status === 'forbidden') {
      throw new LegacyPreflightError(
...
export function classifyDurability(code, knownBindings = new Set()) {
  ...
  const classifyModule = (source) => {
    ...
    if (classification.status === 'forbidden') {
      throw new LegacyPreflightError(`cell import of ${source.value} is forbidden because it exposes kernel control`, source)
    }
...
function classifyPrepared({ code, sourceMap }, program, knownBindings, moduleRewrite, staticModuleReasons) {
  ...
  try {
    ...
    if (error instanceof LegacyPreflightError && error.span !== undefined) {
...
export function prepareLegacyProgram(program, options = {}) {
  ...
  const convenience = reserved.length > 0 ? {
    ...
    collisions: reserved.map(declaration => ({ ...collisionFor(declaration),
      reason: reservedBindings.has(declaration.name)
        ? 'reserved-program-binding-not-shadowable' : 'import-binding-redeclaration' })),
    redeclared: [], rewrites: [],
