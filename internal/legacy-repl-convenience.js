/** Historical legacy-v1 lowering, retained from c51616c8d6c8:repl-convenience.js. */
    ...
    for (const declarator of statement.declarations) {
      ...
      if (existing.length > 0 && existing.length < bindings.length) {
        if (autoSplitRedeclarations) entries.push({ declarator, bindings, existing })
          ...
          rejected.push(...existing.map(binding => ({ ...binding, kind: 'variable',
            reason: 'redeclaration-splitting-disabled' })))
          statementRejected = true
        }
        continue
      }
