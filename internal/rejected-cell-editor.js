...
export function editRunCodeSchema() {
  return {
    name: 'edit_run_code',
    description: 'Prefer a localized correction supplied by a result proving pre-execution rejection when it matches your intent; no recovery context is required. Make a small exact or regular-expression change to the most recent eligible cell captured at dispatch, then run the complete corrected cell. Validation proves syntax/preflight acceptance, not intended behavior. A successful edit becomes the next eligible cell. For successful or partially executed targets, justify a complete-cell rerun using the operation owner\'s retry/idempotence contract and available execution facts; otherwise continue with existing variables in a new run_code cell. This tool does not resume at the error location. Send exactly one atomic edits or regex_edits array, preserving expected_target_call_seq when supplied by a diagnostic. Choose edits for a few unique literal fragments; choose regex_edits when one counted pattern covers repeated fragments. regex_edits replacements use JavaScript template syntax: reference captures as $1/$<name>, never as backslash escapes such as \\1. Every resolved range must be non-overlapping in the original cell.',
    parameters: {
      type: 'object',
      oneOf: [
