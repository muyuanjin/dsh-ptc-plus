  ...
  factory: (require) => {
    ...
    async function apply(ctx) {
      ...
      function BindingAuthorButton({
        ...
        const quickAccess = settings.status === 'ready' && settings.value?.enabled === true
        ...
        return h('span', {
          ...
          !hasDraft && !quickAccess ? null : h(Menu, {
            ...
            onSelect: id => {
              ...
              else if (id === 'manage') setManaging(true)
              else if (id === view.candidateKey) openBindingReview(review, view.candidate)
            },
          }),
          managing && quickAccess && view.reachable ? h(BindingsDialog, { t, callUserBindings, onClose: () => setManaging(false) }) : null,
