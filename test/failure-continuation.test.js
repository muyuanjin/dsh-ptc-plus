...
test('state receipts describe settlement and restore changes the following cell', async t => {
  ...
  assert.match(JSON.stringify(metadata.value), /after cell settlement and journal confirmation, not when the receipt is awaited/)
  assert.match(JSON.stringify(metadata.value), /checkpoint names, not variable names/)
